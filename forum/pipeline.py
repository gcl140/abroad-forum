# def logout_existing_user(strategy, backend, request, *args, **kwargs):
#     if request and hasattr(request, "user") and request.user.is_authenticated:
#         from django.contrib.auth import logout
#         logout(request)


# def logout_existing_user(strategy, backend, *args, **kwargs):
#     request = strategy.request  # Use strategy.request, not the injected `request`
#     if request.user.is_authenticated:
#         from django.contrib.auth import logout
#         logout(request)
#         # Important: Also clear session data or social-auth will think user is logged in
